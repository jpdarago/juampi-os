...
3. ◐ **Continue the UI plan** ([[ui]]): ✅ terminal instance (`9425ed3`, + the
   context-carrying console sink) → ✅ `gfx_surface` (`6876859` + `68c7860` +
   `ac69025`): all drawing is surface-based, `gfx_target` and the global clip are
   ...
   `mu_Context` (`9ee6270`, deferral rule gone) → ✅ `ui.textbox` (`3f04236`).
   (Canvas rename `ui.canvas`→`ui.surface` deferred: breaking to
   raytracer.lua/boing.lua; do it with a deprecation alias when convenient.)
