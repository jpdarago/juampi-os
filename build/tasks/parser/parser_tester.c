...
START_TEST(test_too_long_argument)
{
    ...
    argument_list* l = parse_arguments(program);
    fail_unless(l == NULL);
    fail_unless(!strcmp(get_parse_error(), "Argument too long"));
}
END_TEST
...
START_TEST(test_too_many_arguments)
{
    ...
    argument_list* l = parse_arguments(program);
    fail_unless(l == NULL);
    fail_unless(!strcmp(get_parse_error(), "Too many arguments"));
}
END_TEST
...
START_TEST(test_incorrect_argument)
{
    ...
    argument_list* l = parse_arguments(program);
    fail_unless(l == NULL);
    fail_unless(!strcmp(get_parse_error(), "Invalid character"));
}
END_TEST
...
TFun tests[] = {test_parse_name_only,
