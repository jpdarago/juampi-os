...
#define SYS_getmouse                                                           \
    21 // -> 1 if a mouse is present (else 0); fills a caller int[5] with
       // {x, y, dx, dy, buttons}: absolute position clamped to the screen, the
       // delta since the last call, and the button bitmask (bit0 L/1 R/2 M)
