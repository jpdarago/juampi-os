...
void kmain(void)
{
    ...
    *p = 0xCAFEBABEDEADBEEFull;
    ...
    // render sample glyphs into an offscreen surface, and confirm the
    // rasteriser produced coverage — including partial-coverage (anti-aliased)
    // pixels, which is the point of it over the 8x16 bitmap. Uses its own
    // buffer, so it runs headless; skipped when no font shipped on the disk.
    if (ext2_mounted()) {
        size_t flen = 0;
