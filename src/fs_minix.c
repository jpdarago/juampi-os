...
static disk_position inode_entry_position(ushort inode_num, minix_fs_data* mfd)
{
    disk_position p;
    inode_num--;
...
static int read_inode_entry(ushort inode_number, minix_inode* data,
                            minix_fs_data* mfd)
{
...
static disk_position find_block_ptr(inode* ino, uint block_index, bool create)
{
    ...
    if (block_index < MINIX_BLOCK_PTRS) {
        ...
        p.block = read_data_entry(mi->doubly_indirect_block, ind_block_index);
        if (!p.block) {
            if (!create)
                return disk_pos(0, 0);
...
static disk_position inode_offset_position(inode* ino, uint offset, bool create)
{
    minix_inode* mi = get_disk_inode(ino);
    minix_fs_data* mfd = ino->super_block->fs_data;
...
static int minix_inode_data_read(inode* ino, uint offset, uint bytes,
                                 void* _data)
{
    char* data = _data;
    int processed = 0;
...
static int minix_inode_write_process(inode* ino, uint offset, uint bytes,
                                     void* _data)
{
    char* data = _data;
    int processed = 0;
    ...
    for (; bytes;) {
        ...
        buffered_write(p.block, p.offset, process, ino->inode_number, data);
...
static int minix_inode_data_write(inode* ino, uint offset, uint bytes,
                                  void* data)
{
    int written = minix_inode_write_process(ino, offset, bytes, data);
    if (written < 0) {
        return written;
    }
...
static int minix_read_file(file_object* file, uint bytes, void* _buffer)
{
    ...
    if (bytes > remaining)
        bytes = remaining;
    int read = minix_inode_data_read(file->inode, file_offset, bytes, _buffer);
    file->file_read_offset += read;
    return read;
}

static int minix_write_file(file_object* file, uint bytes, void* _buffer)
{
    ...
    int written = minix_inode_data_write(file->inode, offset, bytes, _buffer);
...
static const fs_ops minix_file_file_ops = {.read = minix_read_file,
...
static disk_position check_entries(minix_dir_entry entries[], char* name,
                                   uint block)
{
...
static disk_position minix_lookup_direct(minix_inode* mi, char* name)
{
...
static disk_position minix_lookup_indirect(ushort ind_block, char* name)
{
...
static disk_position minix_lookup_doubly_indirect(ushort dind_block, char* name)
{
    ushort indirect_blocks[MINIX_BLOCK_PTRS];
    disk_position res = disk_pos(0, 0);
    buffered_read(dind_block, 0, sizeof(indirect_blocks), indirect_blocks);
    for (int i = 0; i < MINIX_BLOCK_PTRS; i++) {
...
static disk_position disk_ptr_lookup(minix_inode* mi, char* dir)
{
    ...
    if (mi->doubly_indirect_block) {
        res = minix_lookup_doubly_indirect(mi->doubly_indirect_block, dir);
        if (res.block)
            return res;
    }
    return disk_pos(0, 0);
...
static inode* inode_from_dir_entry(super_block* s, disk_position p)
{
    minix_dir_entry d;
...
static inode* minix_create_subfile(inode* parent, char* name)
{
    ...
    int res = minix_inode_data_write(parent, parent->file_size, sizeof(dir),
                                     &dir);
    if (res != sizeof(dir))
        return NULL;
...
static int minix_mkdir(inode* parent, char* name)
{
    ...
    minix_dir_entry ode = {.name = ".", .inode = new_inode->inode_number};
    minix_dir_entry pde = {.name = "..", .inode = parent->inode_number};
...
static int minix_unlink(inode* parent, char* name)
{
    ...
    if (rmed->inode_type != FS_DIR && rmed->inode_type != FS_FILE)
        return -EDIRNOTEMPTY;

    if (rmed->inode_type == FS_DIR && rmed->file_size > MINIX_DEF_DIRSIZE)
        return -EINVTYPE;

    parent->file_size -= sizeof(minix_dir_entry);

    minix_dir_entry last;
    minix_inode_data_read(parent, parent->file_size, sizeof(last), &last);

    buffered_write(p.block, p.offset, sizeof(last), parent->inode_number,
                   &last);

    memset(&last, 0, sizeof(last));
    minix_inode_data_write(parent, parent->file_size, sizeof(last), &last);
...
static const inode_ops minix_dir_inode_ops = {.lookup = minix_lookup,
                                              .mkdir = minix_mkdir,
                                              .rmdir = minix_unlink,
                                              .create = minix_create,
                                              .unlink = minix_unlink};
...
{
    list_add(&data->open_ptr, &data->super_block->open_inodes);
}
...
static int write_inode_entry(ushort ino_num, minix_inode* ifd,
                             minix_fs_data* mfd)
{
...
static void minix_flush_bitmaps(super_block* s)
{
    ...
    // to disk faster
    minix_fs_data* mfd = s->fs_data;
    buffered_write_several(mfd->inode_bitmap_start_block,
                           mfd->inode_bitmap_blocks + mfd->zone_bitmap_blocks,
                           0, mfd->inode_bitmap.start);
}
...
static void minix_release_blocks(super_block* sb, ushort* entries, uint count,
                                 int depth)
{
    minix_fs_data* mfd = sb->fs_data;
    for (uint i = 0; i < count; i++) {
        ...
        if (depth > 0) {
            ...
            minix_release_blocks(sb, space, MINIX_BLOCK_PTRS, depth - 1);
        }
...
void release_inode_blocks(inode* ino, super_block* sb)
{
    ...
    minix_release_blocks(sb, &diskino->doubly_indirect_block, 1, depth + 1);
}
...
static void minix_init_disk_info(super_block* block, minix_super_block* d)
{
    block->magic = d->magic;
    block->block_size = MINIX_BLOCK_SIZE;
    block->max_file_size = d->max_file_size;

    uint data_zone_size = d->data_zones * (1024 << d->data_zone_size_log);

    block->disk_size = data_zone_size;
    block->is_dirty = false;
}

static void* buffer_from_disk(minix_super_block* d, uint start, uint blocks)
{
...
static void load_minix_specific_metadata(minix_fs_data* m, minix_super_block* d)
{
    m->number_inodes = d->number_inodes;
    m->data_zone_start = d->data_zone_start;
    m->inode_entries_start =
            MINIX_INODEBMAP_STARTB + d->inode_map_bsize + d->zone_map_bsize;
}

static void load_minix_bitmaps(minix_fs_data* m, minix_super_block* d)
{
    uint inodebmap_start = MINIX_INODEBMAP_STARTB;
    uint zonebmap_start = MINIX_ZONEBMAP_STARTB;

    void* bitmaps_buf = buffer_from_disk(
            d, inodebmap_start, d->inode_map_bsize + d->zone_map_bsize);
    ...
    bitset_load(&m->zone_bitmap, (char*)bitmaps_buf + inodebmap_size,
                zonebmap_size);
...
static void read_super_block(minix_super_block* res)
{
    ...
    hdd_read(MINIX_SUPERBLOCK_START_BLOCK, MINIX_SUPERBLOCK_BSIZE, buffer);
