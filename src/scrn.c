...
#include <vargs.h>
#include <irq.h>
// Start position of the video memory
static ushort* video_mem =
        (ushort*)0xb8000; // The video memory starts at address 0xb8000
// Alignment variables
...
bool in_video_mem(uint address)
{
    return address >= 0xb8000 &&
           address <= 0xb8000 + VIDEO_WIDTH * VIDEO_HEIGHT * sizeof(ushort);
}

static int scrn_valid_pos(uchar cursor_row, uchar cursor_col)
{
    return cursor_row < VIDEO_HEIGHT && cursor_col < VIDEO_WIDTH;
}
...
void scrn_shift_up(void)
{
    ...
    memset(video_mem + (VIDEO_HEIGHT - 1) * VIDEO_WIDTH, 0,
           VIDEO_WIDTH * sizeof(ushort));
}
...
void scrn_putc(char c, ushort fmt)
{
    ...
    switch (c) {
    case '\n': // new line with carriage return (go to the left)
    ...
    case '\t': // tab
        ...
        break;
    default: // character (assumed printable).
...
// Prints at the indicated position: Returns 0 if everything is
// fine or -1 in case of error
int scrn_pos_print(uchar row, uchar col, const char* msg)
{
    uint eflags = irq_cli();
    int ret = 0;
    if (scrn_valid_pos(row, col)) {
        uchar prev_row = scrn_getrow(), prev_col = scrn_getcol();
